...
import com.revethq.documents.dto.PresignedDownloadResponse
import com.revethq.documents.dto.ProblemDetail
import com.revethq.documents.dto.UpdateDocumentRequest
...
class DocumentResource
    ...
    constructor(
            ...
            APIResponse(
                ...
                content = [
                    Content(
                        ...
                        schema = Schema(implementation = com.revethq.documents.dto.PageDTO::class),
                    ),
                ],
            ),
        ...
        fun listDocuments(
            ...
            val warnings = mutableListOf<com.revethq.documents.dto.ProblemDetail>()
            ...
            val sort =
                sortField?.let { field ->
                    ...
                    com.revethq.documents.domain
                        .Sort(field, ascending)
                }
            val pageRequest =
                com.revethq.documents.domain
                    .PageRequest(validatedPage, validatedSize, sort)

            val documentPage =
                documentService.getDocumentsPaginated(
            ...
            val pageDTO =
                com.revethq.documents.api.mapper.PageDTOMapper.toDTO(
                    documentPage,
        ...
        @Operation(
            summary = "Get download URL for latest document version",
            description = "Generate a presigned download URL for the latest version of a document",
        )
        @APIResponses(
            APIResponse(
                responseCode = "200",
                ...
                content = [
                    Content(
                        mediaType = MediaType.APPLICATION_JSON,
                        schema = Schema(implementation = com.revethq.documents.dto.PresignedDownloadResponse::class),
                    ),
                ],
            ),
        ...
        fun downloadLatestVersion(
            ...
            return Response
                .ok(
                    com.revethq.documents.dto.PresignedDownloadResponse(
        ...
        fun createDocument(request: com.revethq.documents.dto.CreateDocumentRequest): Response =
            try {
                val document =
        ...
        fun updateDocument(
            ...
            @Parameter(description = "Document UUID")
            uuid: UUID,
            request: com.revethq.documents.dto.UpdateDocumentRequest,
        ): Response {
            return try {
                val document =
                    documentService.updateDocumentByUuid(
                        uuid = uuid,
                        name = request.name,
                        categoryId = request.categoryId,
                        mime = request.mime,
        ...
        fun addTag(
            ...
            return try {
                val document =
                    documentService.addTagToDocumentByUuid(uuid, request.tag)
                        ?: return Response
                            .status(Response.Status.NOT_FOUND)
                            .entity(mapOf("error" to "Document not found"))
                            .build()
        ...
        fun removeTag(
            ...
            val document =
                documentService.removeTagFromDocumentByUuid(uuid, tag)
                    ?: return Response
                        .status(Response.Status.NOT_FOUND)
                        .entity(mapOf("error" to "Document not found"))
                        .build()
